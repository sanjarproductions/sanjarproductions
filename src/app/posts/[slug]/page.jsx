...
const PostPage = dynamic(
  () =>
    Promise.resolve(({ slug: initialSlug }) => {
      ...
      const title =
        page.properties?.["Doc name"]?.title?.[0]?.plain_text || "Untitled";
      const category = page.properties?.["Category"]?.select;
      const createdBy = page.properties?.["Created by"]?.people?.[0];
      const createdTime =
        page.properties?.["Created time"]?.created_time || page.created_time;
        ...
        switch (block.type) {
          ...
          case "quote":
            return (
              <blockquote
                key={block.id}
                className="border-l-4 border-gray-300 pl-4 mb-4 italic"
              >
